...
import ListClients from "./list_client";
import { useGetClientsQuery } from "@/hooks/apis/ClientApi";
...
const ClientsTable = () => {
  const {
    ...
    isError,
    error,
  } = useGetClientsQuery(); // Utilise useGetChauffeursQuery pour obtenir les utilisateurs
  ...
  if (isLoading) {
    ...
    return <>{users && <ListClients users={users} />} </>; // Utilisez la propriété users au lieu de Chauffeurs
  }
